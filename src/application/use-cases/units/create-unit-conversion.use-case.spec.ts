...
import { UnitNotFoundException, UnitConversionAlreadyExistsException } from "../../../domain/exceptions/unit.exceptions";
...
describe("CreateUnitConversionUseCase", () => {
  ...
  let unitConversionRepository: any;
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: UNIT_CONVERSION_REPOSITORY, useValue: unitConversionRepository },
      ],
    }).compile();

    useCase = module.get<CreateUnitConversionUseCase>(CreateUnitConversionUseCase);
  });
  ...
  it("should throw ProductNotFoundException if product does not exist", async () => {
    ...
    await expect(useCase.execute(request)).rejects.toThrow(ProductNotFoundException);
  });
  ...
  it("should throw UnitNotFoundException if fromUnit does not exist", async () => {
    ...
    await expect(useCase.execute(request)).rejects.toThrow(UnitNotFoundException);
  });

  it("should throw UnitConversionAlreadyExistsException if conversion already exists", async () => {
    productRepository.findById.mockResolvedValue(new ProductEntity({ id: 1 }));
    unitRepository.findByCode.mockResolvedValue(new UnitEntity({ code: "any" }));
    unitConversionRepository.findByProductAndUnits.mockResolvedValue(
      new UnitConversionEntity({ id: 1 }),
    );

    await expect(useCase.execute(request)).rejects.toThrow(UnitConversionAlreadyExistsException);
  });
});
